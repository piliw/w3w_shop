...
<article class="page-container">
	<form action="/brand/{{$data->id}}" method="post" class="form form-horizontal" id="form-member-add" enctype="multipart/form-data">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>品牌名字：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{$data->name}}" placeholder="品牌简称" id="name" name="name" required>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>品牌描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="content" cols="5" rows="10" class="textarea valid" placeholder="品牌介绍..." id="textarea" maxlength="50" required>{{$data->content}}</textarea><p class="textarea-numberbar maxlength"><em class="textarea-length">0</em>/50</p>
			</div>
		</div>
		<div class="row cl">
		...
		<input type="hidden" name="ylogo" value="{{$data->logo}}">
		{{method_field('PUT')}}
 		{{csrf_field()}}
 		<br>
		<div class="row cl">
...
$(function(){
	...
	$('#textarea').keyup(function(){
		...
		if(txtleng>50){
			...
			var a=50;
			$('.textarea-length').text(a-txtleng);
		}
	});
